 ...
 * 蓝牙设备连接相关建监听和管理
 */
public class DBBluetoothManager {
    private static final String TAG = "DBBluetoothManager";
    ...
    private class BluetoothHeadsetBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)) {
                ...
                Log.d(TAG, "### BT ACTION_BOND_STATE_CHANGED ##");
...
//                addBondDevicesToListView();
            ...
            if (action.equals(BluetoothDevice.ACTION_FOUND)) {
                ...
                Log.d(TAG, "搜索蓝压设备" + device.getName() + "连接状态：" + device.getBondState());
...
//                }
            ...
            if (action.equals(BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED)) {
                Log.d(TAG, "蓝牙设备Name发生改变");
            }
            if (action.equals(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE)) {
                Log.d(TAG, "请求用户选择是否使该蓝牙能被扫描 ");
            }
            ...
            if (action.equals(BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED)) {
                ...
                Log.d(TAG, "广播 BluetoothHeadsetBroadcastReceiver.onReceive: "
                ...
                if (state == BluetoothHeadset.STATE_CONNECTED) {
                    ...
                    Log.e(TAG, "广播 connection_state_change  state connected");
                    ...
                    stopScoAudio();
                    Log.e(TAG, "广播 connection_state_change  state disconnect");
...
//                    audioManager.setBluetoothScoOn(false);
    ...
    public void start() {
        ...
        bluetoothHeadsetFilter.addAction(BluetoothDevice.ACTION_BOND_STATE_CHANGED);
        //配对开始时，配对成功时
        bluetoothHeadsetFilter.addAction(BluetoothDevice.ACTION_ACL_CONNECTED);
        //配对时，发起连接

        bluetoothHeadsetFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);

        bluetoothHeadsetFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECTED);
        //配对结束时，断开连接

        bluetoothHeadsetFilter.addAction(BluetoothDevice.ACTION_PAIRING_REQUEST);
        //配对请求（Android.bluetooth.device.action.PAIRING_REQUEST）
        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_DISCOVERY_STARTED);
        //开始搜索
        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_DISCOVERY_FINISHED);
        //搜索结束。重新搜索时，会先终止搜索
        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);

        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_STATE_CHANGED);
        //本机开启、关闭蓝牙开关


        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED);
        //更改蓝牙名称，打开蓝牙时，可能会调用多次

        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);

        bluetoothHeadsetFilter.addAction(BluetoothAdapter.ACTION_REQUEST_ENABLE);
    ...
    public boolean startScoAudio() {
        ...
        if (this.bluetoothHeadset != null) {
            try {
                this.bluetoothHeadset.getClass().getMethod("connectAudio", new Class[0]).invoke(this.bluetoothHeadset, new Object[0]);
                Log.e(TAG, "connect audio  bluetooth success");
            } catch (Exception e) {
                Log.e(TAG, "connect audio  bluetooth exception");
                e.printStackTrace();
            }
        }
...
//        startTimer();
    ...
    public void stopScoAudio() {
        if (audioManager.isBluetoothScoOn()) {
            ...
            if (this.bluetoothHeadset != null) {
                try {
                    this.bluetoothHeadset.getClass().getMethod("disconnectAudio", new Class[0]).invoke(this.bluetoothHeadset, new Object[0]);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    protected void findConnectedDevice() {
        ...
        if (!pairedDevices.isEmpty()) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                ...
                Log.d(TAG, " name=" + device.getName() + ", address=" + device.getAddress() + " state" + isBlueDeviceConnect(device));
            }
        }
    }
    ...
    private boolean isBlueDeviceConnect(BluetoothDevice device) {
